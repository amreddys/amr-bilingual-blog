        ...
        <div class="row">
            <div class="col-12">
                ...
                <div class="row">
                 ...
                 <form method="put" :action="submitLocation" @submit.prevent="onSubmit" @change="form.errors.clear($event.target.name)" @keydown="form.errors.clear($event.target.name)" @reset="onReset">
                <div class="row">
                    <div class="col-12 col-md-6">
                ...
                <div class="row col-4 offset-4 mt-3">
                    <srbuttons class="mx-auto">Update Post</srbuttons>
                </div>
                </form>
            </div>
        </div>
...
export default {
    props: ['post','category_idcsv'],
    data() {
            return {
            form: new Form({
                en_title: this.post.title_en,
                tel_title: this.post.title_tel,
                en_content: this.post.content_en,
                tel_content: this.post.content_tel,
                excerpt_en: this.post.excerpt_en,
                excerpt_tel: this.post.excerpt_tel,
                status: this.post.status,
                comments_enabled: this.post.comments_enabled,
                featured_image: this.post.featured_image,
                categories: {},
            }),submitLocation: siteUrl + '/posts/'+this.post.id,
    ...
    methods: {
          onSubmit() {
             if(this.method='post')
              this.form.put(this.submitLocation)
                 .then(response => {
                      this.form.reset()
          ...
          loadCategories()
          {
              ...
              .then((response) => {
                ...
                if(this.category_csv != '')
                {
                    let cats = this.category_idcsv.split(", ")
                    for(let i = 0; i<cats.length; i++)
                        this.form.categories[cats[i]] = true;
                }
              })
